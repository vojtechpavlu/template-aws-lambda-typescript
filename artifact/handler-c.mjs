...
var handlerC = (data) => {
  ...
  return {
    statusCode: 400,
    body: JSON.stringify({
      message: `Invalid Note: ${NoteSchema.safeParse(data).error?.issues.map((issue) => issue.message).join(", ")}`
    })
  };
};
var notes = [
...
var results = notes.map((note) => handlerC(note));
